...
use shadowquic::config::{AuthUser, DirectOutCfg, DnsCfg, SocksClientCfg, SocksServerCfg};
...
use shadowquic::{direct::outbound::DirectOut, socks::inbound::start_socks_inbound};
...
#[tokio::test]
async fn test_tcp() {
    let socks_server = "127.0.0.1:1034";
    let target_addr = "127.0.0.1";
    let target_port = 1453;
...
async fn spawn_socks() {
    ...
    let sq_client = SocksClient::new("sq".to_string(), SocksClientCfg {
        addr: "127.0.0.1:1071".into(),
    ...
    start_socks_inbound(
        ...
        client_router,
        SocksServerCfg {
            bind_addr: "127.0.0.1:1034".parse().unwrap(),
            users: vec![],
        },
    )
    .await
