...
fn parse_host_port(path: &str) -> Result<SocksAddr, SError> {
    ...
    let host = parts[1];
    Ok(SocksAddr::from_domain(host.to_string(), port))
}
...
fn parse_url(path: &str, headers: &[httparse::Header]) -> Result<SocksAddr, SError> {
    if path.starts_with("http://") || path.starts_with("https://") {
        ...
        // 0    1 2        3
        if url_parts.len() >= 3 {
            return parse_host_port(url_parts[2]);
        }
    }
    ...
    for header in headers {
        if header.name.eq_ignore_ascii_case("Host") {
            ...
            if let Ok(addr) = parse_host_port(host) {
                return Ok(addr);
            }
            return Ok(SocksAddr::from_domain(host.to_string(), 80));
        }
    }
